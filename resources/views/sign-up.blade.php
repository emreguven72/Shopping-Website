...
<div class="container mx-auto p-5">
    ...
    <div class="mx-auto border-2 w-1/3 shadow-lg my-20">
        <div class="flex flex-col p-5">
            <h1 class="my-10 text-3xl mx-auto">Kayıt ol</h1>
            <form class="flex flex-col" action="{{route('signup')}}" method="POST">
                @csrf
                <input type="email" class="h-10 my-3 placeholder:text-gray-600 p-2 bg-gray-100 rounded-lg" name="email" placeholder="E-posta">
                <input type="text" class="h-10 my-3 placeholder:text-gray-600 p-2 bg-gray-100 rounded-lg" name="username" placeholder="Kullanıcı adı">
                <input type="password" class="h-10 my-3 placeholder:text-gray-600 p-2 bg-gray-100 rounded-lg" name="password" placeholder="Şifre">
                <button type="submit" class="bg-purple-600 h-10 rounded-lg text-gray-50 my-3">Kayıt ol</button>
            </form>
        </div>
    </div><!-- Login Form -->
</div>
